...
define build
	@go build -ldflags " \
	-X github.com/laocai/oexchain/cmd/utils.commit=$(shell cat commit_hash.txt) \
	-X github.com/laocai/oexchain/cmd/utils.date=$(shell date '+%Y-%m-%d-%H:%M:%S') \
	-X 'github.com/laocai/oexchain/cmd/utils.goversion=$(shell go version)'" \
